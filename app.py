...
# T1
# http://127.0.0.1:5000/api/card/new?card_number=123123&owner_id=13124&owner_name=ola&bank_name=ola&due_date=2028-09-12&franchise=ola&payment_day=09&monthly_fee=12000&interest_rate=3
@app.route('/api/card/new')
def insert_credit_card():
    """
    Crea una nueva tarjeta de crédito y la inserta en la base de datos.
    """
    try:
        ...
        # Insertar tarjeta de crédito en la base de datos
        ControllerCreditCard.insert_credit_card(credit_card)

        # Buscar la tarjeta de crédito recién creada
...
# T2
# http://127.0.0.1:5000/api/simulate/purchase?card_number=123123&purchase_amount=50000&payments=24
@app.route('/api/simulate/purchase')
def simulate_purchase():
    """
    Simula una compra con una tarjeta de crédito y devuelve la cuota mensual y el interés total
    """
    try:
        # Obtener parámetros de la solicitud
        card_number = request.args["card_number"]
        ...
        # Calcular la cuota mensual y el interés total
...
# T3
# http://127.0.0.1:5000/api/simulate/saving?card_number=123123&purchase_amount=50000&payments=24
@app.route('/api/simulate/saving')
def simulate_planned_saving():
    """
    Simula un plan de ahorro con una tarjeta de crédito y devuelve el número de meses necesarios para alcanzar el objetivo
    """
    try:
        # Obtener parámetros de la solicitud
        card_number = request.args["card_number"]
        ...
        # Calcular la cuota mensual y el plan de ahorro
        monthly_amount = CreditCard.calc_monthly_payment(search_credit_card, purchase_amount, payments)
...
# T4
# http://127.0.0.1:5000/api/purchase/new?card_number=123123&purchase_amount=200000&&payments=36&purchase_date=2023-09-11
@app.route('/api/purchase/new')
def simulate_payment_plan():
    """
    Simula una nueva compra con una tarjeta de crédito y almacena el plan de pagos en la base de datos
    """
    try:
        # Obtener parámetros de la solicitud
        card_number = request.args["card_number"]
        ...
        # Insertar el plan de pagos en la base de datos
